...
def get_rec(row):
    if row['Мои_заказы'] == 0:
        ...
        if (row['Моя_доля_рынка_%'] < 5 and
            row['%  прироста выручки'] >= min_growth and
            row['Мой_процент_выкупа'] >= min_buyout):  # ← ИСПРАВЛЕНО
            return "🚀 Усиление"
        elif row['Мой_процент_выкупа'] < 70:  # ← ИСПРАВЛЕНО
...
# -------------------------------
# ФИЛЬТР ПО РЕКОМЕНДАЦИЯМ ← ДОБАВЛЕНО
# -------------------------------
...
st.dataframe(
    display_df[[
        ...
        'Продавцы с заказами', 'Мои_заказы', 'Моя_доля_рынка_%', 'Мой_процент_выкупа', 'Рекомендация'  # ← ИСПРАВЛЕНО
    ]],

...
# ОКРУГЛЕНИЕ до целого (остаётся int → сортировка работает)
result['Мой_процент_выкупа'] = result['Мой_процент_выкупа'].round(0).astype(int)
...
# === КОНФИГУРАЦИЯ СТОЛБЦОВ ДЛЯ ЛЕВОГО ВЫРАВНИВАНИЯ И ФОРМАТА ===
column_config = {
    "Предмет": st.column_config.TextColumn("Предмет", width="medium"),
    "Юрлица": st.column_config.TextColumn("Юрлица", width="small"),
    "Процент выкупа": st.column_config.NumberColumn("Процент выкупа", format="%d"),
    "Выручка, ₽": st.column_config.TextColumn("Выручка, ₽", width="small"),
    "Ср. выкупы/карточки": st.column_config.NumberColumn("Ср. выкупы/карточки", format="%d"),
    "Количество_запросов": st.column_config.NumberColumn("Количество_запросов", format="%d"),
    "Монополизация, %": st.column_config.NumberColumn("Монополизация, %", format="%.1f"),
    "Продавцы с заказами": st.column_config.NumberColumn("Продавцы с заказами", format="%d"),
    "Мои_заказы": st.column_config.NumberColumn("Мои_заказы", format="%d"),
    "Моя_доля_рынка_%": st.column_config.NumberColumn("Моя_доля_рынка_%", format="%.2f"),
    "Мой_процент_выкупа": st.column_config.NumberColumn("Мой_процент_выкупа", format="%d"),
    "Рекомендация": st.column_config.TextColumn("Рекомендация", width="small"),
}
...
if selected_subject:
    ...
    lower_column_config = {
        "Поисковый запрос": st.column_config.TextColumn("Поисковый запрос", width="medium"),
        "Количество запросов": st.column_config.NumberColumn("Количество запросов", format="%d"),
        "Конверсия в корзину": st.column_config.TextColumn("Конверсия в корзину"),
        "Количество запросов (пред.)": st.column_config.NumberColumn("Количество запросов (пред.)", format="%d"),
        "Δ Запросы, %": st.column_config.TextColumn("Δ Запросы, %"),
        "Конверсия в заказ": st.column_config.TextColumn("Конверсия в заказ"),
        "Заказали товаров": st.column_config.NumberColumn("Заказали товаров", format="%d"),
        "Заказали товаров (пред.)": st.column_config.NumberColumn("Заказали товаров (пред.)", format="%d"),
        "Δ Заказы, %": st.column_config.TextColumn("Δ Заказы, %"),
    }

    st.dataframe(
        q_filtered[lower_columns],
        use_container_width=True,
        hide_index=True,
        column_config=lower_column_config
    )
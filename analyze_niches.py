...
# Агрегация продаж с КОРРЕКТНЫМ выкупом
...
sales_agg['Мой_процент_выкупа'] = (
    sales_agg['Мои_выкупы'] / sales_agg['Мои_заказы'].replace(0, pd.NA) * 100
).round(2)
...
result['Мои_выкупы'] = result['Мои_выкупы'].fillna(0)
result['Мой_процент_выкупа'] = result['Мой_процент_выкупа'].fillna(0)
result['Количество_запросов'] = result['Количество_запросов'].fillna(0)
...
# Доля рынка
result['Моя_доля_рынка_%'] = (result['Мои_заказы'] / result['Выручка, ₽'].replace(0, pd.NA) * 100).round(2)
...
for col in ['Выручка, ₽', 'Количество_запросов', 'Мои_заказы', 'Мои_выкупы']:
    if col in display_df.columns:
        display_df[col] = display_df[col].apply(format_number)

# Форматирование процентных значений
for col in ['Монополизация, %', 'Моя_доля_рынка_%', 'Мой_процент_выкупа', '% прироста выручки']:
    if col in display_df.columns:
...
if 'Предмет' in result.columns and not result['Предмет'].empty:
    ...
    if selected_subject:
        ...
        if not q.empty:
            ...
            required_cols = ['Количество запросов', 'Количество запросов (предыдущий период)',
            ...
            q['Δ Запросы, %'] = (
                ...
                q['Количество запросов (предыдущий период)'].replace(0, pd.NA) * 100
            ).round(1)
            
            q['Δ Заказы, %'] = (
                (q['Заказали товаров'] - q['Заказали товаров (предыдущий период)']) /
                q['Заказали товаров (предыдущий период)'].replace(0, pd.NA) * 100
            ).round(1)
            
            q = q.sort_values('Заказали товаров', ascending=False)
            
            # Форматирование для отображения
            display_cols = ['Поисковый запрос', 'Количество запросов', 
                          'Количество запросов (предыдущий период)', 'Δ Запросы, %',
                          'Заказали товаров', 'Заказали товаров (предыдущий период)', 'Δ Заказы, %']
    ...
    st.info("Нет данных о предметах для анализа")
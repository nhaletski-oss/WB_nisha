...
# -------------------------------
# Гарантируем числовой тип для заказов
...
# Корректный расчёт выкупа — только где есть заказы
...
base['Количество запросов'] = base['Количество запросов'].fillna(0)
...
for col in ['Мои_заказы', 'Мои_выкупы', 'Мои_товары', 'Мой_выкуп_процент']:
...
def get_rec(row):
    if row['Мои_заказы'] == 0:
        if (row['Количество запросов'] >= min_queries and
...
# Подготовка к отображению: форматируем ТОЛЬКО для вывода
df_display = result.copy()
df_display['Количество запросов'] = df_display['Количество запросов'].apply(format_number)
df_display['Мои_заказы'] = df_display['Мои_заказы'].apply(format_number)

st.dataframe(
    df_display[[
        'Предмет', 'Юрлица', 'Выручка, ₽', 'Количество запросов', 'Монополизация, %',
...
if selected_subject:
    ...
    # Гарантируем числовой тип для сортировки
